...
3. **Benutzerdefinierte Konfiguration** (`CLAUDIO_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`opencode.json` im Projekt) – projektspezifische Einstellungen
5. **`.claudio` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`CLAUDIO_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.claudio` und `~/.config/opencode` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Geben Sie mithilfe der Umgebungsvariablen `CLAUDIO_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `CLAUDIO_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.claudio` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.claudio` geladen, sodass es deren Einstellungen **überschreiben** kann.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/opencode/agents/` oder `.claudio/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/opencode/commands/` oder `.claudio/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.claudio/plugins/` oder `~/.config/opencode/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
