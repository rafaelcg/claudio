...
OpenCode Desktop startet im Hintergrund einen lokalen OpenCode-Server (`claudio-cli`-Sidecar).
...
Wenn du `CLAUDIO_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `CLAUDIO_PORT` zurueck (oder waehle einen freien Port) und starte neu.
