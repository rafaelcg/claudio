...
3. **Özel yapılandırma** (`CLAUDIO_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `opencode.json`) - projeye özgü ayarlar
5. **`.claudio` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`CLAUDIO_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.claudio` ve `~/.config/opencode` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
`CLAUDIO_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`CLAUDIO_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.claudio` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.claudio` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.
...
Agent'ları `~/.config/opencode/agents/` veya `.claudio/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Komutları `~/.config/opencode/commands/` veya `.claudio/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
Eklenti dosyalarını `.claudio/plugins/` veya `~/.config/opencode/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
