...
opencode Desktop arka planda yerel bir opencode sunucusu (`claudio-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.
...
Ortamda `CLAUDIO_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `CLAUDIO_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
