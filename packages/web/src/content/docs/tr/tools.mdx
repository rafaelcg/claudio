...
:::note
Bu araç yalnızca `CLAUDIO_EXPERIMENTAL_LSP_TOOL=true` (veya `CLAUDIO_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca opencode provider kullanılırken veya `CLAUDIO_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.
