...
- `edit` - Cannot modify existing files, except for files located at `.claudio/plans/*.md` to detail the plan itself
...
Create mode files in `.claudio/modes/` for project-specific modes or `~/.config/opencode/modes/` for global modes:
