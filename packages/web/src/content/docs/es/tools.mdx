...
:::note
Esta herramienta solo está disponible cuando `CLAUDIO_EXPERIMENTAL_LSP_TOOL=true` (o `CLAUDIO_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenCode o cuando la variable de entorno `CLAUDIO_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenCode:

```bash
CLAUDIO_ENABLE_EXA=1 opencode
```
