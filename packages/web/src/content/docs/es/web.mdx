...
:::caution
Si no se establece `CLAUDIO_SERVER_PASSWORD`, el servidor no será seguro. Esto está bien para uso local, pero debe configurarse para acceso a la red.
:::
...
Para proteger el acceso, establezca una contraseña utilizando la variable de entorno `CLAUDIO_SERVER_PASSWORD`:

```bash
CLAUDIO_SERVER_PASSWORD=secret opencode web
```

El nombre de usuario predeterminado es `opencode` pero se puede cambiar con `CLAUDIO_SERVER_USERNAME`.
