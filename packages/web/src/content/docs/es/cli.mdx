...
Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad opencode sin la interfaz TUI. Configure `CLAUDIO_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Esto inicia un servidor HTTP y abre un navegador web para acceder a OpenCode a través de una interfaz web. Configure `CLAUDIO_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
| ------------------------------------- | -------- | ------------------------------------------------------------------------------- |
| `CLAUDIO_AUTO_SHARE`                 | booleano | Compartir sesiones automáticamente                                              |
| `CLAUDIO_GIT_BASH_PATH`              | cadena   | Ruta al ejecutable de Git Bash en Windows                                       |
| `CLAUDIO_CONFIG`                     | cadena   | Ruta al archivo de configuración                                                |
| `CLAUDIO_CONFIG_DIR`                 | cadena   | Ruta al directorio de configuración                                             |
| `CLAUDIO_CONFIG_CONTENT`             | cadena   | Contenido de configuración json en línea                                        |
| `CLAUDIO_DISABLE_AUTOUPDATE`         | booleano | Deshabilitar las comprobaciones automáticas de actualizaciones                  |
| `CLAUDIO_DISABLE_PRUNE`              | booleano | Deshabilitar la poda de datos antiguos                                          |
| `CLAUDIO_DISABLE_TERMINAL_TITLE`     | booleano | Deshabilitar las actualizaciones automáticas de títulos de terminal             |
| `CLAUDIO_PERMISSION`                 | cadena   | Configuración de permisos json incorporados                                     |
| `CLAUDIO_DISABLE_DEFAULT_PLUGINS`    | booleano | Deshabilitar complementos predeterminados                                       |
| `CLAUDIO_DISABLE_LSP_DOWNLOAD`       | booleano | Deshabilitar las descargas automáticas del servidor LSP                         |
| `CLAUDIO_ENABLE_EXPERIMENTAL_MODELS` | booleano | Habilitar modelos experimentales                                                |
| `CLAUDIO_DISABLE_AUTOCOMPACT`        | booleano | Deshabilitar la compactación automática de contexto                             |
| `CLAUDIO_DISABLE_CLAUDE_CODE`        | booleano | Deshabilitar la lectura desde `.claude` (mensaje + habilidades)                 |
| `CLAUDIO_DISABLE_CLAUDE_CODE_PROMPT` | booleano | Desactivar lectura `~/.claude/CLAUDE.md`                                        |
| `CLAUDIO_DISABLE_CLAUDE_CODE_SKILLS` | booleano | Deshabilitar la carga `.claude/skills`                                          |
| `CLAUDIO_DISABLE_MODELS_FETCH`       | booleano | Deshabilitar la recuperación de modelos desde fuentes remotas                   |
| `CLAUDIO_FAKE_VCS`                   | cadena   | Proveedor de VCS falso para fines de prueba                                     |
| `CLAUDIO_DISABLE_FILETIME_CHECK`     | booleano | Deshabilite la verificación del tiempo del archivo para optimizarlo             |
| `CLAUDIO_CLIENT`                     | cadena   | Identificador de cliente (por defecto `cli`)                                    |
| `CLAUDIO_ENABLE_EXA`                 | booleano | Habilitar las herramientas de búsqueda web de Exa                               |
| `CLAUDIO_SERVER_PASSWORD`            | cadena   | Habilite la autenticación básica para `serve`/`web`                             |
| `CLAUDIO_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `opencode`) |
| `CLAUDIO_MODELS_URL`                 | cadena   | URL personalizada para buscar la configuración de modelos                       |
...
| Variable                                        | Type     | Description                                                |
| ----------------------------------------------- | -------- | ---------------------------------------------------------- |
| `CLAUDIO_EXPERIMENTAL`                         | booleano | Habilitar todas las funciones experimentales               |
| `CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY`          | booleano | Habilitar descubrimiento de íconos                         |
| `CLAUDIO_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | booleano | Deshabilitar copia al seleccionar en TUI                   |
| `CLAUDIO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | número   | Tiempo de espera predeterminado para comandos bash en ms   |
| `CLAUDIO_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | número   | Tokens de salida máximos para respuestas LLM               |
| `CLAUDIO_EXPERIMENTAL_FILEWATCHER`             | booleano | Habilite el observador de archivos para todo el directorio |
| `CLAUDIO_EXPERIMENTAL_OXFMT`                   | booleano | Habilitar el formateador oxfmt                             |
| `CLAUDIO_EXPERIMENTAL_LSP_TOOL`                | booleano | Habilitar herramienta experimental LSP                     |
| `CLAUDIO_EXPERIMENTAL_DISABLE_FILEWATCHER`     | booleano | Deshabilitar el observador de archivos                     |
| `CLAUDIO_EXPERIMENTAL_EXA`                     | booleano | Habilitar funciones experimentales de Exa                  |
| `CLAUDIO_EXPERIMENTAL_LSP_TY`                  | booleano | Habilitar Habilitar TY LSP para archivos python            |
| `CLAUDIO_EXPERIMENTAL_MARKDOWN`                | booleano | Habilitar funciones de Markdown experimentales             |
| `CLAUDIO_EXPERIMENTAL_PLAN_MODE`               | booleano | Habilitar modo de plan                                     |
