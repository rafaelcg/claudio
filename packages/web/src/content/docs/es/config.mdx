...
3. **Configuración personalizada** (`CLAUDIO_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`opencode.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.claudio`** - agentes, comandos, complementos
6. **Configuración en línea** (`CLAUDIO_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.claudio` y `~/.config/opencode` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `CLAUDIO_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `CLAUDIO_CONFIG_DIR`
variable de entorno. En este directorio se buscarán agentes, comandos,
modos y complementos como el directorio estándar `.claudio`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.claudio`, por lo que **puede anular** su configuración.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/opencode/agents/` o `.claudio/agents/`. [Más información aquí](/docs/agents).
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/opencode/commands/` o `.claudio/commands/`. [Más información aquí](/docs/commands).
...
Coloque los archivos de complemento en `.claudio/plugins/` o `~/.config/opencode/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
