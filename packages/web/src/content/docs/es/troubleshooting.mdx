...
OpenCode Desktop ejecuta un servidor OpenCode local (el sidecar `claudio-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
Si tiene `CLAUDIO_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `CLAUDIO_PORT` (o elija un puerto libre) y reinicie.
