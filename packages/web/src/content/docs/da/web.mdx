...
:::caution
Hvis `CLAUDIO_SERVER_PASSWORD` ikke er indstillet, vil serveren være usikret. Dette er fint til lokal brug, men bør indstilles til netværksadgang.
:::
...
For at beskytte adgangen skal du angive en adgangskode ved hjælp af `CLAUDIO_SERVER_PASSWORD` miljøvariablen:

```bash
CLAUDIO_SERVER_PASSWORD=secret opencode web
```

Brugernavnet er som standard `opencode`, men kan ændres med `CLAUDIO_SERVER_USERNAME`.
