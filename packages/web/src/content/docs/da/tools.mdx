...
:::note
Dette verktøyet er bare tilgængelig når `CLAUDIO_EXPERIMENTAL_LSP_TOOL=true` (eller `CLAUDIO_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenCode-leverandøren eller når miljøvariabelen `CLAUDIO_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
