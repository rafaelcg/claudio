...
OpenCode Desktop kører en lokal OpenCode-server (`claudio-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.
...
Hvis du har `CLAUDIO_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `CLAUDIO_PORT` (eller vælg en ledig port) og start på ny.
