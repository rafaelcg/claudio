...
3. **Tilpasset konfiguration** (`CLAUDIO_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`opencode.json` i projekt) - projektspecifikke indstillinger
5. **`.claudio` mapper** - agent, kommandoer, plugins
6. **Inline config** (`CLAUDIO_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.claudio` og `~/.config/opencode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `CLAUDIO_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `CLAUDIO_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.claudio` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.claudio` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/opencode/agents/` eller `.claudio/agents/`. [Learn more here](/docs/agents).
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/opencode/commands/` eller `.claudio/commands/`. [Learn more here](/docs/commands).
...
Placer plugin-filer i `.claudio/plugins/` eller `~/.config/opencode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
