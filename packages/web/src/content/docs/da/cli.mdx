...
Dette starter en HTTP-server, der giver API-adgang til opencode-funktionalitet uden TUI-grænsefladen. Indstil `CLAUDIO_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Dette starter en HTTP-server og åbner en webbrowser for at få adgang til OpenCode via en webgrænseflade. Indstil `CLAUDIO_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
| ------------------------------------ | ------- | --------------------------------------------------------------------- |
| `CLAUDIO_AUTO_SHARE`                 | boolean | Del automatisk session                                                |
| `CLAUDIO_GIT_BASH_PATH`              | string  | Sti til Git Bash eksekverbar på Windows                               |
| `CLAUDIO_CONFIG`                     | string  | Sti til konfigurationsfil                                             |
| `CLAUDIO_TUI_CONFIG`                 | string  | Sti til TUI-konfigurationsfil                                         |
| `CLAUDIO_CONFIG_DIR`                 | string  | Sti til konfigurationsmappe                                           |
| `CLAUDIO_CONFIG_CONTENT`             | string  | Indbygget json-konfigurationsindhold                                  |
| `CLAUDIO_DISABLE_AUTOUPDATE`         | boolean | Deaktiver automatisk opdateringskontrol                               |
| `CLAUDIO_DISABLE_PRUNE`              | boolean | Deaktiver beskæring af gamle data                                     |
| `CLAUDIO_DISABLE_TERMINAL_TITLE`     | boolean | Deaktiver automatisk opdatering af terminaltitel                      |
| `CLAUDIO_PERMISSION`                 | string  | Indbygget json-tilladelseskonfiguration                               |
| `CLAUDIO_DISABLE_DEFAULT_PLUGINS`    | boolean | Deaktiver standard plugins                                            |
| `CLAUDIO_DISABLE_LSP_DOWNLOAD`       | boolean | Deaktiver automatisk LSP-serverdownloads                              |
| `CLAUDIO_ENABLE_EXPERIMENTAL_MODELS` | boolean | Aktive eksperimentelle modeller                                       |
| `CLAUDIO_DISABLE_AUTOCOMPACT`        | boolean | Deaktiver automatisk kontekstkomprimering                             |
| `CLAUDIO_DISABLE_CLAUDE_CODE`        | boolean | Deaktiver læsning fra `.claude` (prompt + færdigheder)                |
| `CLAUDIO_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Deaktiver læsning `~/.claude/CLAUDE.md`                               |
| `CLAUDIO_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Deaktiver indlæsning af `.claude/skills`                              |
| `CLAUDIO_DISABLE_MODELS_FETCH`       | boolean | Deaktivering af modeller fra eksterne kilder                          |
| `CLAUDIO_FAKE_VCS`                   | string  | Falsk VCS-udbyder til testformål                                      |
| `CLAUDIO_DISABLE_FILETIME_CHECK`     | boolean | Deaktiver filtidskontrol for optimering                               |
| `CLAUDIO_CLIENT`                     | string  | Klient-id (standard til `cli`)                                        |
| `CLAUDIO_ENABLE_EXA`                 | boolean | Aktiver Exa-websøgeværktøjer                                          |
| `CLAUDIO_SERVER_PASSWORD`            | string  | Aktiver grundlæggende godkendelse for `serve`/`web`                   |
| `CLAUDIO_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `opencode`) |
| `CLAUDIO_MODELS_URL`                 | string  | Brugerdefineret URL til hentning af modelkonfiguration                |
...
| Variabel                                       | Skriv   | Beskrivelse                                |
| ---------------------------------------------- | ------- | ------------------------------------------ |
| `CLAUDIO_EXPERIMENTAL`                         | boolean | Aktiver alle eksperimentelle funktioner    |
| `CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Aktiver ikonopdagelse                      |
| `CLAUDIO_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Deaktiver kopi ved valg i TUI              |
| `CLAUDIO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | nummer  | Standard timeout for bash-kommandoer i ms  |
| `CLAUDIO_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | nummer  | Maks. output-tokens for LLM-svar           |
| `CLAUDIO_EXPERIMENTAL_FILEWATCHER`             | boolean | Aktiver filovervågning for hele dir        |
| `CLAUDIO_EXPERIMENTAL_OXFMT`                   | boolean | Aktiver oxfmt formatter                    |
| `CLAUDIO_EXPERIMENTAL_LSP_TOOL`                | boolean | Aktive eksperimenter LSP værktøj           |
| `CLAUDIO_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Deaktiver filovervågning                   |
| `CLAUDIO_EXPERIMENTAL_EXA`                     | boolean | Aktive eksperimenter Exa-funktioner        |
| `CLAUDIO_EXPERIMENTAL_LSP_TY`                  | boolean | Aktiver TY LSP for python-filer            |
| `CLAUDIO_EXPERIMENTAL_MARKDOWN`                | boolean | Aktive eksperimentelle markdown-funktioner |
| `CLAUDIO_EXPERIMENTAL_PLAN_MODE`               | boolean | Aktiver plantilstand                       |
