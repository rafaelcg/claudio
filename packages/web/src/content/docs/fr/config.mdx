...
3. **Configuration personnalisée** (`CLAUDIO_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`opencode.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.claudio`** - agents, commandes, plugins
6. **Configuration en ligne** (`CLAUDIO_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.claudio` et `~/.config/opencode` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `CLAUDIO_CONFIG`.

```bash
export CLAUDIO_CONFIG=/path/to/my/custom-config.json
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `CLAUDIO_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.claudio`, et devrait suivre la même structure.

```bash
export CLAUDIO_CONFIG_DIR=/path/to/my/config-directory
opencode run "Hello world"
```

Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.claudio`, il **peut donc remplacer** leurs paramètres.
...
Utilisez `CLAUDIO_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/opencode/agents/` ou `.claudio/agents/`. [En savoir plus ici](/docs/agents).
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/opencode/commands/` ou `.claudio/commands/`. [En savoir plus ici](/docs/commands).
...
Placez les fichiers du plugin dans `.claudio/plugins/` ou `~/.config/opencode/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
