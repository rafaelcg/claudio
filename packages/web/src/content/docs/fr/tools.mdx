...
:::note
Cet outil n'est disponible que lorsque `CLAUDIO_EXPERIMENTAL_LSP_TOOL=true` (ou `CLAUDIO_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur OpenCode ou lorsque la variable d'environnement `CLAUDIO_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).
