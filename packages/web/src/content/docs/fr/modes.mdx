...
- `edit` - Impossible de modifier les fichiers existants, à l'exception des fichiers situés à `.claudio/plans/*.md` pour détailler le plan lui-même
...
Créez des fichiers de mode dans `.claudio/modes/` pour les modes spécifiques au projet ou `~/.config/opencode/modes/` pour les modes globaux :

```markdown title=".claudio/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
