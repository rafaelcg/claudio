...
Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'OpenCode sans l'interface TUI. Définissez `CLAUDIO_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à OpenCode via une interface Web. Définissez `CLAUDIO_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
| ------------------------------------ | ------- | --------------------------------------------------------------------------------- |
| `CLAUDIO_AUTO_SHARE`                 | booléen | Partager automatiquement des sessions                                             |
| `CLAUDIO_GIT_BASH_PATH`              | chaîne  | Chemin vers l'exécutable Git Bash sur Windows                                     |
| `CLAUDIO_CONFIG`                     | chaîne  | Chemin d'accès au fichier de configuration                                        |
| `CLAUDIO_TUI_CONFIG`                 | chaîne  | Chemin d'accès au fichier de configuration TUI                                    |
| `CLAUDIO_CONFIG_DIR`                 | chaîne  | Chemin d'accès au répertoire de configuration                                     |
| `CLAUDIO_CONFIG_CONTENT`             | chaîne  | Contenu de configuration JSON en ligne                                            |
| `CLAUDIO_DISABLE_AUTOUPDATE`         | booléen | Désactiver les vérifications automatiques des mises à jour                        |
| `CLAUDIO_DISABLE_PRUNE`              | booléen | Désactiver le nettoyage des anciennes données                                     |
| `CLAUDIO_DISABLE_TERMINAL_TITLE`     | booléen | Désactiver les mises à jour automatiques des titres du terminal                   |
| `CLAUDIO_PERMISSION`                 | chaîne  | Configuration des autorisations JSON intégrées                                    |
| `CLAUDIO_DISABLE_DEFAULT_PLUGINS`    | booléen | Désactiver les plugins par défaut                                                 |
| `CLAUDIO_DISABLE_LSP_DOWNLOAD`       | booléen | Désactiver les téléchargements automatiques du serveur LSP                        |
| `CLAUDIO_ENABLE_EXPERIMENTAL_MODELS` | booléen | Activer les modèles expérimentaux                                                 |
| `CLAUDIO_DISABLE_AUTOCOMPACT`        | booléen | Désactiver le compactage automatique du contexte                                  |
| `CLAUDIO_DISABLE_CLAUDE_CODE`        | booléen | Désactiver la lecture de `.claude` (prompt + compétences)                         |
| `CLAUDIO_DISABLE_CLAUDE_CODE_PROMPT` | booléen | Désactiver la lecture `~/.claude/CLAUDE.md`                                       |
| `CLAUDIO_DISABLE_CLAUDE_CODE_SKILLS` | booléen | Désactiver le chargement de `.claude/skills`                                      |
| `CLAUDIO_DISABLE_MODELS_FETCH`       | booléen | Désactiver la récupération de modèles à partir de sources distantes               |
| `CLAUDIO_FAKE_VCS`                   | chaîne  | Faux fournisseur VCS à des fins de test                                           |
| `CLAUDIO_DISABLE_FILETIME_CHECK`     | booléen | Désactiver la vérification de l'heure des fichiers pour l'optimisation            |
| `CLAUDIO_CLIENT`                     | chaîne  | Identifiant du client (par défaut `cli`)                                          |
| `CLAUDIO_ENABLE_EXA`                 | booléen | Activer les outils de recherche Web Exa                                           |
| `CLAUDIO_SERVER_PASSWORD`            | chaîne  | Activer l'authentification de base pour `serve`/`web`                             |
| `CLAUDIO_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `opencode`) |
| `CLAUDIO_MODELS_URL`                 | chaîne  | URL personnalisé pour récupérer la configuration des modèles                      |
...
| Variables                                      | Type    | Description                                                     |
| ---------------------------------------------- | ------- | --------------------------------------------------------------- |
| `CLAUDIO_EXPERIMENTAL`                         | booléen | Activer toutes les fonctionnalités expérimentales               |
| `CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY`          | booléen | Activer la découverte d'icônes                                  |
| `CLAUDIO_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | booléen | Désactiver la copie lors de la sélection dans TUI               |
| `CLAUDIO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | numéro  | Délai d'expiration par défaut pour les commandes bash en ms     |
| `CLAUDIO_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | numéro  | Nombre maximum de jetons de sortie pour les réponses LLM        |
| `CLAUDIO_EXPERIMENTAL_FILEWATCHER`             | booléen | Activer l'observateur de fichiers pour l'ensemble du répertoire |
| `CLAUDIO_EXPERIMENTAL_OXFMT`                   | booléen | Activer le formateur oxfmt                                      |
| `CLAUDIO_EXPERIMENTAL_LSP_TOOL`                | booléen | Activer l'outil expérimental LSP                                |
| `CLAUDIO_EXPERIMENTAL_DISABLE_FILEWATCHER`     | booléen | Désactiver l'observateur de fichiers                            |
| `CLAUDIO_EXPERIMENTAL_EXA`                     | booléen | Activer les fonctionnalités Exa expérimentales                  |
| `CLAUDIO_EXPERIMENTAL_LSP_TY`                  | booléen | Activer TY LSP pour les fichiers python                         |
| `CLAUDIO_EXPERIMENTAL_MARKDOWN`                | booléen | Activer les fonctionnalités Markdown expérimentales             |
| `CLAUDIO_EXPERIMENTAL_PLAN_MODE`               | booléen | Activer le mode plan                                            |
