...
이 명령은 TUI 없이 opencode 기능에 접근할 수 있는 HTTP 서버를 시작합니다. `CLAUDIO_SERVER_PASSWORD`를 설정하면 HTTP basic auth가 활성화됩니다(기본 사용자명: `opencode`).
...
이 명령은 HTTP 서버를 시작하고 웹 브라우저를 열어 웹 인터페이스로 OpenCode에 접속합니다. `CLAUDIO_SERVER_PASSWORD`를 설정하면 HTTP basic auth가 활성화됩니다(기본 사용자명: `opencode`).
...
| 변수                                  | 타입    | 설명                                           |
| ------------------------------------- | ------- | ---------------------------------------------- |
| `CLAUDIO_AUTO_SHARE`                 | boolean | 세션 자동 공유                                 |
| `CLAUDIO_GIT_BASH_PATH`              | string  | Windows에서 Git Bash 실행 파일 경로            |
| `CLAUDIO_CONFIG`                     | string  | 설정 파일 경로                                 |
| `CLAUDIO_CONFIG_DIR`                 | string  | 설정 디렉터리 경로                             |
| `CLAUDIO_CONFIG_CONTENT`             | string  | 인라인 JSON 설정 내용                          |
| `CLAUDIO_DISABLE_AUTOUPDATE`         | boolean | 자동 업데이트 확인 비활성화                    |
| `CLAUDIO_DISABLE_PRUNE`              | boolean | 오래된 데이터 정리(prune) 비활성화             |
| `CLAUDIO_DISABLE_TERMINAL_TITLE`     | boolean | 터미널 제목 자동 업데이트 비활성화             |
| `CLAUDIO_PERMISSION`                 | string  | 인라인 JSON 권한 설정                          |
| `CLAUDIO_DISABLE_DEFAULT_PLUGINS`    | boolean | 기본 플러그인 비활성화                         |
| `CLAUDIO_DISABLE_LSP_DOWNLOAD`       | boolean | LSP 서버 자동 다운로드 비활성화                |
| `CLAUDIO_ENABLE_EXPERIMENTAL_MODELS` | boolean | 실험적 모델 활성화                             |
| `CLAUDIO_DISABLE_AUTOCOMPACT`        | boolean | 자동 컨텍스트 컴팩션 비활성화                  |
| `CLAUDIO_DISABLE_CLAUDE_CODE`        | boolean | `.claude`(프롬프트 + 스킬) 읽기 비활성화       |
| `CLAUDIO_DISABLE_CLAUDE_CODE_PROMPT` | boolean | `~/.claude/CLAUDE.md` 읽기 비활성화            |
| `CLAUDIO_DISABLE_CLAUDE_CODE_SKILLS` | boolean | `.claude/skills` 로드 비활성화                 |
| `CLAUDIO_DISABLE_MODELS_FETCH`       | boolean | 원격 소스에서 모델 목록 가져오기 비활성화      |
| `CLAUDIO_FAKE_VCS`                   | string  | 테스트용 가짜 VCS provider                     |
| `CLAUDIO_DISABLE_FILETIME_CHECK`     | boolean | 최적화를 위한 파일 시간 검사 비활성화          |
| `CLAUDIO_CLIENT`                     | string  | 클라이언트 식별자(기본값: `cli`)               |
| `CLAUDIO_ENABLE_EXA`                 | boolean | Exa 웹 검색 도구 활성화                        |
| `CLAUDIO_SERVER_PASSWORD`            | string  | `serve`/`web` 기본 인증 활성화                 |
| `CLAUDIO_SERVER_USERNAME`            | string  | 기본 인증 사용자명 오버라이드(기본 `opencode`) |
| `CLAUDIO_MODELS_URL`                 | string  | 모델 설정을 가져올 사용자 지정 URL             |
...
| 변수                                            | 타입    | 설명                             |
| ----------------------------------------------- | ------- | -------------------------------- |
| `CLAUDIO_EXPERIMENTAL`                         | boolean | 모든 실험 기능 활성화            |
| `CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | 아이콘 탐색 활성화               |
| `CLAUDIO_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | TUI에서 선택 시 복사 비활성화    |
| `CLAUDIO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | bash 명령 기본 타임아웃(ms)      |
| `CLAUDIO_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | LLM 응답 최대 출력 토큰 수       |
| `CLAUDIO_EXPERIMENTAL_FILEWATCHER`             | boolean | 전체 디렉터리 파일 감시 활성화   |
| `CLAUDIO_EXPERIMENTAL_OXFMT`                   | boolean | oxfmt 포매터 활성화              |
| `CLAUDIO_EXPERIMENTAL_LSP_TOOL`                | boolean | 실험적 LSP 도구 활성화           |
| `CLAUDIO_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | 파일 감시 비활성화               |
| `CLAUDIO_EXPERIMENTAL_EXA`                     | boolean | 실험적 Exa 기능 활성화           |
| `CLAUDIO_EXPERIMENTAL_LSP_TY`                  | boolean | python 파일에 대해 TY LSP 활성화 |
| `CLAUDIO_EXPERIMENTAL_MARKDOWN`                | boolean | 실험적 Markdown 기능 활성화      |
| `CLAUDIO_EXPERIMENTAL_PLAN_MODE`               | boolean | Plan mode 활성화                 |
