...
opencode 데스크톱은 배경에서 로컬 opencode 서버 (`claudio-cli` sidecar)를 실행합니다. 대부분의 문제는 misbehaving 플러그인, 손상된 캐시, 또는 나쁜 서버 설정에 의해 발생합니다.
...
`CLAUDIO_PORT`가 있는 경우, 데스크탑 앱은 로컬 서버의 포트를 사용하려고 합니다.

- `CLAUDIO_PORT` (또는 무료 포트를 선택) 및 재시작.
