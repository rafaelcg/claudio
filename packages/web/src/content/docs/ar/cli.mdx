...
يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف opencode دون واجهة TUI. اضبط `CLAUDIO_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opencode`).
...
يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى OpenCode عبر واجهة ويب. اضبط `CLAUDIO_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opencode`).
...
| المتغير                              | النوع   | الوصف                                                     |
| ------------------------------------ | ------- | --------------------------------------------------------- |
| `CLAUDIO_AUTO_SHARE`                 | boolean | مشاركة الجلسات تلقائيا                                    |
| `CLAUDIO_GIT_BASH_PATH`              | string  | مسار ملف Git Bash التنفيذي على Windows                    |
| `CLAUDIO_CONFIG`                     | string  | مسار ملف التهيئة                                          |
| `CLAUDIO_CONFIG_DIR`                 | string  | مسار دليل التهيئة                                         |
| `CLAUDIO_CONFIG_CONTENT`             | string  | محتوى تهيئة JSON مُضمّن                                   |
| `CLAUDIO_DISABLE_AUTOUPDATE`         | boolean | تعطيل التحقق التلقائي من التحديثات                        |
| `CLAUDIO_DISABLE_PRUNE`              | boolean | تعطيل تنقية البيانات القديمة                              |
| `CLAUDIO_DISABLE_TERMINAL_TITLE`     | boolean | تعطيل تحديث عنوان terminal تلقائيا                        |
| `CLAUDIO_PERMISSION`                 | string  | تهيئة أذونات JSON مُضمّنة                                 |
| `CLAUDIO_DISABLE_DEFAULT_PLUGINS`    | boolean | تعطيل الإضافات الافتراضية                                 |
| `CLAUDIO_DISABLE_LSP_DOWNLOAD`       | boolean | تعطيل تنزيل خوادم LSP تلقائيا                             |
| `CLAUDIO_ENABLE_EXPERIMENTAL_MODELS` | boolean | تفعيل النماذج التجريبية                                   |
| `CLAUDIO_DISABLE_AUTOCOMPACT`        | boolean | تعطيل ضغط السياق تلقائيا                                  |
| `CLAUDIO_DISABLE_CLAUDE_CODE`        | boolean | تعطيل القراءة من `.claude` (الموجه + المهارات)            |
| `CLAUDIO_DISABLE_CLAUDE_CODE_PROMPT` | boolean | تعطيل قراءة `~/.claude/CLAUDE.md`                         |
| `CLAUDIO_DISABLE_CLAUDE_CODE_SKILLS` | boolean | تعطيل تحميل `.claude/skills`                              |
| `CLAUDIO_DISABLE_MODELS_FETCH`       | boolean | تعطيل جلب النماذج من مصادر بعيدة                          |
| `CLAUDIO_FAKE_VCS`                   | string  | مزود VCS وهمي لأغراض الاختبار                             |
| `CLAUDIO_DISABLE_FILETIME_CHECK`     | boolean | تعطيل التحقق من وقت الملف لتحسين الأداء                   |
| `CLAUDIO_CLIENT`                     | string  | معرّف العميل (الافتراضي `cli`)                            |
| `CLAUDIO_ENABLE_EXA`                 | boolean | تفعيل أدوات بحث الويب من Exa                              |
| `CLAUDIO_SERVER_PASSWORD`            | string  | تفعيل المصادقة الأساسية لخادمي `serve`/`web`              |
| `CLAUDIO_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `opencode`) |
| `CLAUDIO_MODELS_URL`                 | string  | رابط مخصص لجلب تهيئة النماذج                              |
...
| المتغير                                        | النوع   | الوصف                                       |
| ---------------------------------------------- | ------- | ------------------------------------------- |
| `CLAUDIO_EXPERIMENTAL`                         | boolean | تفعيل جميع الميزات التجريبية                |
| `CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | تفعيل اكتشاف الأيقونات                      |
| `CLAUDIO_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | تعطيل النسخ عند التحديد في TUI              |
| `CLAUDIO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | المهلة الافتراضية لأوامر bash بالميلي ثانية |
| `CLAUDIO_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | الحد الأقصى لرموز المخرجات لاستجابات LLM    |
| `CLAUDIO_EXPERIMENTAL_FILEWATCHER`             | boolean | تفعيل مراقب الملفات لكامل الدليل            |
| `CLAUDIO_EXPERIMENTAL_OXFMT`                   | boolean | تفعيل مُنسّق oxfmt                          |
| `CLAUDIO_EXPERIMENTAL_LSP_TOOL`                | boolean | تفعيل أداة LSP تجريبية                      |
| `CLAUDIO_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | تعطيل مراقب الملفات                         |
| `CLAUDIO_EXPERIMENTAL_EXA`                     | boolean | تفعيل ميزات Exa التجريبية                   |
| `CLAUDIO_EXPERIMENTAL_LSP_TY`                  | boolean | تمكين TY LSP لملفات python                  |
| `CLAUDIO_EXPERIMENTAL_MARKDOWN`                | boolean | تفعيل ميزات markdown تجريبية                |
| `CLAUDIO_EXPERIMENTAL_PLAN_MODE`               | boolean | تفعيل وضع الخطة                             |
