...
:::note
هذه الأداة متاحة فقط عند ضبط `CLAUDIO_EXPERIMENTAL_LSP_TOOL=true` (أو `CLAUDIO_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenCode أو عند ضبط متغير البيئة `CLAUDIO_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).
