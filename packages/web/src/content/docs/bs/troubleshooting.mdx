...
OpenCode Desktop pokreće lokalni OpenCode server (`claudio-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
Ako ste postavili `CLAUDIO_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `CLAUDIO_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
