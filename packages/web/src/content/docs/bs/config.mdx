...
3. **Prilagođena konfiguracija** (`CLAUDIO_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`opencode.json` u projektu) - postavke specifične za projekat
5. **`.claudio` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`CLAUDIO_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.claudio` i `~/.config/opencode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `CLAUDIO_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CLAUDIO_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.claudio` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.claudio`, tako da **može nadjačati** njihove postavke.
...
Koristite `CLAUDIO_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/opencode/agents/` ili `.claudio/agents/`. [Saznajte više ovdje](/docs/agents).
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/opencode/commands/` ili `.claudio/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.claudio/plugins/` ili `~/.config/opencode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
