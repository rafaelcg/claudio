...
Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti OpenCode-a bez TUI interfejsa. Postavite `CLAUDIO_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Ovo pokreće HTTP server i otvara web pretraživač za pristup OpenCode-u preko web interfejsa. Postavite `CLAUDIO_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
| Varijabla                            | Tip     | Opis                                                               |
| ------------------------------------ | ------- | ------------------------------------------------------------------ |
| `CLAUDIO_AUTO_SHARE`                 | boolean | Automatski dijeli sesije                                           |
| `CLAUDIO_GIT_BASH_PATH`              | string  | Putanja do Git Bash izvršne datoteke na Windows-u                  |
| `CLAUDIO_CONFIG`                     | string  | Putanja do konfiguracijskog fajla                                  |
| `CLAUDIO_TUI_CONFIG`                 | string  | Putanja do TUI konfiguracijskog fajla                              |
| `CLAUDIO_CONFIG_DIR`                 | string  | Putanja do konfiguracijskog direktorija                            |
| `CLAUDIO_CONFIG_CONTENT`             | string  | Inline json konfiguracijski sadržaj                                |
| `CLAUDIO_DISABLE_AUTOUPDATE`         | boolean | Onemogući automatske provjere ažuriranja                           |
| `CLAUDIO_DISABLE_PRUNE`              | boolean | Onemogući brisanje (pruning) starih podataka                       |
| `CLAUDIO_DISABLE_TERMINAL_TITLE`     | boolean | Onemogući automatsko ažuriranje naslova terminala                  |
| `CLAUDIO_PERMISSION`                 | string  | Inline json konfiguracija dozvola                                  |
| `CLAUDIO_DISABLE_DEFAULT_PLUGINS`    | boolean | Onemogući podrazumijevane dodatke (plugins)                        |
| `CLAUDIO_DISABLE_LSP_DOWNLOAD`       | boolean | Onemogući automatsko preuzimanje LSP servera                       |
| `CLAUDIO_ENABLE_EXPERIMENTAL_MODELS` | boolean | Omogući eksperimentalne modele                                     |
| `CLAUDIO_DISABLE_AUTOCOMPACT`        | boolean | Onemogući automatsko sažimanje konteksta                           |
| `CLAUDIO_DISABLE_CLAUDE_CODE`        | boolean | Onemogući čitanje iz `.claude` (prompt + vještine)                 |
| `CLAUDIO_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Onemogući čitanje `~/.claude/CLAUDE.md`                            |
| `CLAUDIO_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Onemogući učitavanje `.claude/skills`                              |
| `CLAUDIO_DISABLE_MODELS_FETCH`       | boolean | Onemogući dohvaćanje modela iz udaljenih izvora                    |
| `CLAUDIO_FAKE_VCS`                   | string  | Lažni VCS provajder za potrebe testiranja                          |
| `CLAUDIO_DISABLE_FILETIME_CHECK`     | boolean | Onemogući provjeru vremena datoteke radi optimizacije              |
| `CLAUDIO_CLIENT`                     | string  | Identifikator klijenta (zadano na `cli`)                           |
| `CLAUDIO_ENABLE_EXA`                 | boolean | Omogući Exa alate za web pretraživanje                             |
| `CLAUDIO_SERVER_PASSWORD`            | string  | Omogući osnovnu autentifikaciju za `serve`/`web`                   |
| `CLAUDIO_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `opencode`) |
| `CLAUDIO_MODELS_URL`                 | string  | Prilagođeni URL za dohvaćanje konfiguracije modela                 |
...
| Varijabla                                      | Tip     | Opis                                              |
| ---------------------------------------------- | ------- | ------------------------------------------------- |
| `CLAUDIO_EXPERIMENTAL`                         | boolean | Omogući sve eksperimentalne funkcije              |
| `CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Omogući otkrivanje ikona                          |
| `CLAUDIO_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Onemogući kopiranje pri odabiru u TUI             |
| `CLAUDIO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Zadano vremensko ograničenje za bash naredbe u ms |
| `CLAUDIO_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Maksimalni izlazni tokeni za LLM odgovore         |
| `CLAUDIO_EXPERIMENTAL_FILEWATCHER`             | boolean | Omogući praćenje datoteka za cijeli direktorij    |
| `CLAUDIO_EXPERIMENTAL_OXFMT`                   | boolean | Omogući oxfmt formatter                           |
| `CLAUDIO_EXPERIMENTAL_LSP_TOOL`                | boolean | Omogući eksperimentalni LSP alat                  |
| `CLAUDIO_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Onemogući praćenje datoteka                       |
| `CLAUDIO_EXPERIMENTAL_EXA`                     | boolean | Omogući eksperimentalne Exa funkcije              |
| `CLAUDIO_EXPERIMENTAL_LSP_TY`                  | boolean | Omogući TY LSP za python datoteke                 |
| `CLAUDIO_EXPERIMENTAL_MARKDOWN`                | boolean | Omogući eksperimentalne Markdown funkcije         |
| `CLAUDIO_EXPERIMENTAL_PLAN_MODE`               | boolean | Omogući Plan mod                                  |
