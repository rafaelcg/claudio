...
:::caution
Ako `CLAUDIO_SERVER_PASSWORD` nije postavljen, server će biti nezaštićen. Ovo je u redu za lokalnu upotrebu, ali bi trebalo biti postavljeno za pristup mreži.
:::
...
Da biste zaštitili pristup, postavite lozinku koristeći varijablu okruženja `CLAUDIO_SERVER_PASSWORD`:

```bash
CLAUDIO_SERVER_PASSWORD=secret opencode web
```

Korisničko ime podrazumevano je `opencode`, ali se može promeniti sa `CLAUDIO_SERVER_USERNAME`.
