...
:::note
Ovaj alat je dostupan samo kada je `CLAUDIO_EXPERIMENTAL_LSP_TOOL=true` (ili `CLAUDIO_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCode provajdera ili kada je varijabla `CLAUDIO_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).
