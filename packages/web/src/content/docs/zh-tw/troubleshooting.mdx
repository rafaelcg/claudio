...
OpenCode Desktop 會在背景執行一個本地 OpenCode 伺服器（即 `claudio-cli` 附屬程序）。大多數問題是由外掛異常、快取損壞或錯誤的伺服器設定引起的。
...
如果你在環境中設定了 `CLAUDIO_PORT`，桌面應用程式將嘗試使用該連接埠作為本地伺服器連接埠。

- 取消設定 `CLAUDIO_PORT`（或選擇一個空閒連接埠）並重新啟動。
