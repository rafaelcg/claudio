...
此指令啟動一個 HTTP 伺服器，提供對 OpenCode 功能的 API 存取，無需 TUI 介面。設定 `CLAUDIO_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `opencode`）。
...
此指令啟動一個 HTTP 伺服器並開啟瀏覽器，透過 Web 介面存取 OpenCode。設定 `CLAUDIO_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `opencode`）。
...
| 變數                                 | 類型    | 說明                                        |
| ------------------------------------ | ------- | ------------------------------------------- |
| `CLAUDIO_AUTO_SHARE`                 | boolean | 自動分享工作階段                            |
| `CLAUDIO_GIT_BASH_PATH`              | string  | Windows 上 Git Bash 可執行檔的路徑          |
| `CLAUDIO_CONFIG`                     | string  | 設定檔路徑                                  |
| `CLAUDIO_TUI_CONFIG`                 | string  | TUI 設定檔路徑                              |
| `CLAUDIO_CONFIG_DIR`                 | string  | 設定目錄路徑                                |
| `CLAUDIO_CONFIG_CONTENT`             | string  | 內嵌 JSON 設定內容                          |
| `CLAUDIO_DISABLE_AUTOUPDATE`         | boolean | 停用自動更新檢查                            |
| `CLAUDIO_DISABLE_PRUNE`              | boolean | 停用舊資料清理                              |
| `CLAUDIO_DISABLE_TERMINAL_TITLE`     | boolean | 停用自動終端機標題更新                      |
| `CLAUDIO_PERMISSION`                 | string  | 內嵌 JSON 權限設定                          |
| `CLAUDIO_DISABLE_DEFAULT_PLUGINS`    | boolean | 停用預設外掛程式                            |
| `CLAUDIO_DISABLE_LSP_DOWNLOAD`       | boolean | 停用 LSP 伺服器自動下載                     |
| `CLAUDIO_ENABLE_EXPERIMENTAL_MODELS` | boolean | 啟用實驗性模型                              |
| `CLAUDIO_DISABLE_AUTOCOMPACT`        | boolean | 停用自動上下文壓縮                          |
| `CLAUDIO_DISABLE_CLAUDE_CODE`        | boolean | 停用讀取 `.claude`（提示詞 + 技能）         |
| `CLAUDIO_DISABLE_CLAUDE_CODE_PROMPT` | boolean | 停用讀取 `~/.claude/CLAUDE.md`              |
| `CLAUDIO_DISABLE_CLAUDE_CODE_SKILLS` | boolean | 停用載入 `.claude/skills`                   |
| `CLAUDIO_DISABLE_MODELS_FETCH`       | boolean | 停用從遠端來源擷取模型                      |
| `CLAUDIO_FAKE_VCS`                   | string  | 用於測試目的的模擬 VCS 供應商               |
| `CLAUDIO_DISABLE_FILETIME_CHECK`     | boolean | 停用檔案時間檢查最佳化                      |
| `CLAUDIO_CLIENT`                     | string  | 用戶端識別碼（預設為 `cli`）                |
| `CLAUDIO_ENABLE_EXA`                 | boolean | 啟用 Exa 網路搜尋工具                       |
| `CLAUDIO_SERVER_PASSWORD`            | string  | 為 `serve`/`web` 啟用基本認證               |
| `CLAUDIO_SERVER_USERNAME`            | string  | 覆寫基本認證使用者名稱（預設為 `opencode`） |
| `CLAUDIO_MODELS_URL`                 | string  | 自訂模型設定擷取 URL                        |
...
| 變數                                           | 類型    | 說明                            |
| ---------------------------------------------- | ------- | ------------------------------- |
| `CLAUDIO_EXPERIMENTAL`                         | boolean | 啟用所有實驗性功能              |
| `CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | 啟用圖示探索                    |
| `CLAUDIO_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | 停用 TUI 中的選取即複製         |
| `CLAUDIO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | bash 指令的預設逾時時間（毫秒） |
| `CLAUDIO_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | LLM 回應的最大輸出 Token 數     |
| `CLAUDIO_EXPERIMENTAL_FILEWATCHER`             | boolean | 啟用整個目錄的檔案監看器        |
| `CLAUDIO_EXPERIMENTAL_OXFMT`                   | boolean | 啟用 oxfmt 格式化器             |
| `CLAUDIO_EXPERIMENTAL_LSP_TOOL`                | boolean | 啟用實驗性 LSP 工具             |
| `CLAUDIO_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | 停用檔案監看器                  |
| `CLAUDIO_EXPERIMENTAL_EXA`                     | boolean | 啟用實驗性 Exa 功能             |
| `CLAUDIO_EXPERIMENTAL_LSP_TY`                  | boolean | 為 python 檔案啟用 TY LSP       |
| `CLAUDIO_EXPERIMENTAL_MARKDOWN`                | boolean | 啟用實驗性 Markdown 功能        |
| `CLAUDIO_EXPERIMENTAL_PLAN_MODE`               | boolean | 啟用計畫模式                    |
