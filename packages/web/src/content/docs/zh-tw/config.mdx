...
3. **自訂設定**（`CLAUDIO_CONFIG` 環境變數）- 自訂覆寫
4. **專案設定**（專案中的 `opencode.json`）- 專案特定設定
5. **`.claudio` 目錄** - 代理、指令、外掛程式
6. **內嵌設定**（`CLAUDIO_CONFIG_CONTENT` 環境變數）- 執行時覆寫

這意味著專案設定可以覆寫全域預設值，全域設定可以覆寫遠端組織預設值。

:::note
`.claudio` 和 `~/.config/opencode` 目錄的子目錄使用**複數名稱**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。為了向後相容，也支援單數名稱（例如 `agent/`）。
:::
...
使用 `CLAUDIO_CONFIG` 環境變數指定自訂設定檔路徑。
...
使用 `CLAUDIO_CONFIG_DIR` 環境變數指定自訂設定目錄。該目錄會像標準 `.claudio` 目錄一樣被搜尋代理、指令、模式和外掛程式，並且應遵循相同的結構。
...
自訂目錄在全域設定和 `.claudio` 目錄之後載入，因此**可以覆寫**它們的設定。
...
使用 `CLAUDIO_TUI_CONFIG` 指向自訂 TUI 設定檔。
...
您還可以使用 `~/.config/opencode/agents/` 或 `.claudio/agents/` 中的 Markdown 檔案定義代理。[在此了解更多](/docs/agents)。
...
您還可以使用 `~/.config/opencode/commands/` 或 `.claudio/commands/` 中的 Markdown 檔案定義指令。[在此了解更多](/docs/commands)。
...
將外掛程式檔案放置在 `.claudio/plugins/` 或 `~/.config/opencode/plugins/` 中。您還可以透過 `plugin` 選項從 npm 載入外掛程式。
