...
O opencode Desktop executa um servidor opencode local (o sidecar `claudio-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
Se você tiver `CLAUDIO_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `CLAUDIO_PORT` (ou escolha uma porta livre) e reinicie.
