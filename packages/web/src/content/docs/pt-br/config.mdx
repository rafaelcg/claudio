...
3. **Configuração personalizada** (`CLAUDIO_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`opencode.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.claudio`** - agentes, comandos, plugins
6. **Configuração inline** (`CLAUDIO_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.claudio` e `~/.config/opencode` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `CLAUDIO_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `CLAUDIO_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.claudio`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.claudio`, então ele **pode substituir** suas configurações.
...
Use `CLAUDIO_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/opencode/agents/` ou `.claudio/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/opencode/commands/` ou `.claudio/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.claudio/plugins/` ou `~/.config/opencode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
