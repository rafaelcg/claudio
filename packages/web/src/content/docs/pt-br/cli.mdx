...
Isso inicia um servidor HTTP que fornece acesso à funcionalidade do opencode sem a interface TUI. Defina `CLAUDIO_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
Isso inicia um servidor HTTP e abre um navegador para acessar o opencode através de uma interface web. Defina `CLAUDIO_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
| ------------------------------------ | ------- | --------------------------------------------------------------------- |
| `CLAUDIO_AUTO_SHARE`                 | boolean | Compartilhar sessões automaticamente                                  |
| `CLAUDIO_GIT_BASH_PATH`              | string  | Caminho para o executável do Git Bash no Windows                      |
| `CLAUDIO_CONFIG`                     | string  | Caminho para o arquivo de configuração                                |
| `CLAUDIO_CONFIG_DIR`                 | string  | Caminho para o diretório de configuração                              |
| `CLAUDIO_CONFIG_CONTENT`             | string  | Conteúdo de configuração json inline                                  |
| `CLAUDIO_DISABLE_AUTOUPDATE`         | boolean | Desabilitar verificações de atualização automática                    |
| `CLAUDIO_DISABLE_PRUNE`              | boolean | Desabilitar a poda de dados antigos                                   |
| `CLAUDIO_DISABLE_TERMINAL_TITLE`     | boolean | Desabilitar atualizações automáticas do título do terminal            |
| `CLAUDIO_PERMISSION`                 | string  | Configuração de permissões json inline                                |
| `CLAUDIO_DISABLE_DEFAULT_PLUGINS`    | boolean | Desabilitar plugins padrão                                            |
| `CLAUDIO_DISABLE_LSP_DOWNLOAD`       | boolean | Desabilitar downloads automáticos do servidor LSP                     |
| `CLAUDIO_ENABLE_EXPERIMENTAL_MODELS` | boolean | Habilitar modelos experimentais                                       |
| `CLAUDIO_DISABLE_AUTOCOMPACT`        | boolean | Desabilitar compactação automática de contexto                        |
| `CLAUDIO_DISABLE_CLAUDE_CODE`        | boolean | Desabilitar leitura de `.claude` (prompt + habilidades)               |
| `CLAUDIO_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Desabilitar leitura de `~/.claude/CLAUDE.md`                          |
| `CLAUDIO_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Desabilitar carregamento de `.claude/skills`                          |
| `CLAUDIO_DISABLE_MODELS_FETCH`       | boolean | Desabilitar busca de modelos de fontes remotas                        |
| `CLAUDIO_FAKE_VCS`                   | string  | Provedor VCS falso para fins de teste                                 |
| `CLAUDIO_DISABLE_FILETIME_CHECK`     | boolean | Desabilitar verificação de tempo de arquivo para otimização           |
| `CLAUDIO_CLIENT`                     | string  | Identificador do cliente (padrão é `cli`)                             |
| `CLAUDIO_ENABLE_EXA`                 | boolean | Habilitar ferramentas de busca web Exa                                |
| `CLAUDIO_SERVER_PASSWORD`            | string  | Habilitar autenticação básica para `serve`/`web`                      |
| `CLAUDIO_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `opencode`) |
| `CLAUDIO_MODELS_URL`                 | string  | URL personalizada para buscar configuração de modelos                 |
...
| Variável                                       | Tipo    | Descrição                                                 |
| ---------------------------------------------- | ------- | --------------------------------------------------------- |
| `CLAUDIO_EXPERIMENTAL`                         | boolean | Habilitar todos os recursos experimentais                 |
| `CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Habilitar descoberta de ícones                            |
| `CLAUDIO_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Desabilitar cópia ao selecionar no TUI                    |
| `CLAUDIO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Tempo limite padrão para comandos bash em ms              |
| `CLAUDIO_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Máximo de tokens de saída para respostas LLM              |
| `CLAUDIO_EXPERIMENTAL_FILEWATCHER`             | boolean | Habilitar monitoramento de arquivos para todo o diretório |
| `CLAUDIO_EXPERIMENTAL_OXFMT`                   | boolean | Habilitar formatador oxfmt                                |
| `CLAUDIO_EXPERIMENTAL_LSP_TOOL`                | boolean | Habilitar ferramenta LSP experimental                     |
| `CLAUDIO_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Desabilitar monitoramento de arquivos                     |
| `CLAUDIO_EXPERIMENTAL_EXA`                     | boolean | Habilitar recursos experimentais do Exa                   |
| `CLAUDIO_EXPERIMENTAL_LSP_TY`                  | boolean | Habilitar TY LSP para arquivos python                     |
| `CLAUDIO_EXPERIMENTAL_MARKDOWN`                | boolean | Habilitar recursos experimentais de markdown              |
| `CLAUDIO_EXPERIMENTAL_PLAN_MODE`               | boolean | Habilitar modo de plano                                   |
