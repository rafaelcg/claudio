...
OpenCode Desktop 会在后台运行一个本地 OpenCode 服务器（即 `claudio-cli` 附属进程）。大多数问题是由插件异常、缓存损坏或错误的服务器设置引起的。
...
如果你在环境中设置了 `CLAUDIO_PORT`，桌面应用将尝试使用该端口作为本地服务器端口。

- 取消设置 `CLAUDIO_PORT`（或选择一个空闲端口）并重新启动。
