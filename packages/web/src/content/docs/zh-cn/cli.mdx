...
此命令启动一个 HTTP 服务器，提供对 OpenCode 功能的 API 访问，无需 TUI 界面。设置 `CLAUDIO_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `opencode`）。
...
此命令启动一个 HTTP 服务器并打开浏览器，通过 Web 界面访问 OpenCode。设置 `CLAUDIO_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `opencode`）。
...
| 变量                                 | 类型    | 描述                                    |
| ------------------------------------ | ------- | --------------------------------------- |
| `CLAUDIO_AUTO_SHARE`                 | boolean | 自动分享会话                            |
| `CLAUDIO_GIT_BASH_PATH`              | string  | Windows 上 Git Bash 可执行文件的路径    |
| `CLAUDIO_CONFIG`                     | string  | 配置文件路径                            |
| `CLAUDIO_TUI_CONFIG`                 | string  | TUI 配置文件路径                        |
| `CLAUDIO_CONFIG_DIR`                 | string  | 配置目录路径                            |
| `CLAUDIO_CONFIG_CONTENT`             | string  | 内联 JSON 配置内容                      |
| `CLAUDIO_DISABLE_AUTOUPDATE`         | boolean | 禁用自动更新检查                        |
| `CLAUDIO_DISABLE_PRUNE`              | boolean | 禁用旧数据清理                          |
| `CLAUDIO_DISABLE_TERMINAL_TITLE`     | boolean | 禁用自动终端标题更新                    |
| `CLAUDIO_PERMISSION`                 | string  | 内联 JSON 权限配置                      |
| `CLAUDIO_DISABLE_DEFAULT_PLUGINS`    | boolean | 禁用默认插件                            |
| `CLAUDIO_DISABLE_LSP_DOWNLOAD`       | boolean | 禁用 LSP 服务器自动下载                 |
| `CLAUDIO_ENABLE_EXPERIMENTAL_MODELS` | boolean | 启用实验性模型                          |
| `CLAUDIO_DISABLE_AUTOCOMPACT`        | boolean | 禁用自动上下文压缩                      |
| `CLAUDIO_DISABLE_CLAUDE_CODE`        | boolean | 禁用读取 `.claude`（提示词 + 技能）     |
| `CLAUDIO_DISABLE_CLAUDE_CODE_PROMPT` | boolean | 禁用读取 `~/.claude/CLAUDE.md`          |
| `CLAUDIO_DISABLE_CLAUDE_CODE_SKILLS` | boolean | 禁用加载 `.claude/skills`               |
| `CLAUDIO_DISABLE_MODELS_FETCH`       | boolean | 禁用从远程源获取模型                    |
| `CLAUDIO_FAKE_VCS`                   | string  | 用于测试目的的模拟 VCS 提供商           |
| `CLAUDIO_DISABLE_FILETIME_CHECK`     | boolean | 禁用文件时间检查优化                    |
| `CLAUDIO_CLIENT`                     | string  | 客户端标识符（默认为 `cli`）            |
| `CLAUDIO_ENABLE_EXA`                 | boolean | 启用 Exa 网络搜索工具                   |
| `CLAUDIO_SERVER_PASSWORD`            | string  | 为 `serve`/`web` 启用基本认证           |
| `CLAUDIO_SERVER_USERNAME`            | string  | 覆盖基本认证用户名（默认为 `opencode`） |
| `CLAUDIO_MODELS_URL`                 | string  | 自定义模型配置获取 URL                  |
...
| 变量                                           | 类型    | 描述                            |
| ---------------------------------------------- | ------- | ------------------------------- | --- | ------------------------------- | ------- | ------------------------ |
| `CLAUDIO_EXPERIMENTAL`                         | boolean | 启用所有实验性功能              |
| `CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | 启用图标发现                    |
| `CLAUDIO_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | 禁用 TUI 中的选中即复制         |
| `CLAUDIO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | bash 命令的默认超时时间（毫秒） |
| `CLAUDIO_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | LLM 响应的最大输出 Token 数     |
| `CLAUDIO_EXPERIMENTAL_FILEWATCHER`             | boolean | 启用整个目录的文件监听器        |
| `CLAUDIO_EXPERIMENTAL_OXFMT`                   | boolean | 启用 oxfmt 格式化器             |
| `CLAUDIO_EXPERIMENTAL_LSP_TOOL`                | boolean | 启用实验性 LSP 工具             |
| `CLAUDIO_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | 禁用文件监听器                  |
| `CLAUDIO_EXPERIMENTAL_EXA`                     | boolean | 启用实验性 Exa 功能             |
| `CLAUDIO_EXPERIMENTAL_LSP_TY`                  | boolean | 为 python 文件启用 TY LSP       |     | `CLAUDIO_EXPERIMENTAL_MARKDOWN` | boolean | 启用实验性 Markdown 功能 |
| `CLAUDIO_EXPERIMENTAL_PLAN_MODE`               | boolean | 启用计划模式                    |
