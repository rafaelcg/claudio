...
OpenCode Desktop kjører en lokal OpenCode-server (`claudio-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.
...
Hvis du har `CLAUDIO_PORT` satt i miljøet ditt, vil skrivebordsappen prøve å bruke den porten for den lokale serveren.

- Deaktiver `CLAUDIO_PORT` (eller velg en ledig port) og start på nytt.
