...
3. **Egendefinert konfigurasjon** (`CLAUDIO_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`opencode.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.claudio` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`CLAUDIO_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.claudio` og `~/.config/opencode` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `CLAUDIO_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `CLAUDIO_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.claudio` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.claudio`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/opencode/agents/` eller `.claudio/agents/`. [Les mer her](/docs/agents).
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/opencode/commands/` eller `.claudio/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.claudio/plugins/` eller `~/.config/opencode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
