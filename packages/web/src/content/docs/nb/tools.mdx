...
:::note
Dette verktøyet er bare tilgjengelig når `CLAUDIO_EXPERIMENTAL_LSP_TOOL=true` (eller `CLAUDIO_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenCode-leverandøren eller når miljøvariabelen `CLAUDIO_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).
