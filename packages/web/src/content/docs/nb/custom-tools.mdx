...
- Lokalt ved å plassere dem i `.claudio/tools/`-katalogen til prosjektet ditt.
...
```ts title=".claudio/tools/database.ts" {1}
...
```ts title=".claudio/tools/math.ts"
import { tool } from "@claudio-code/plugin"
...
```ts title=".claudio/tools/bash.ts"
import { tool } from "@claudio-code/plugin"
...
```ts title=".claudio/tools/project.ts" {8}
import { tool } from "@claudio-code/plugin"
...
```python title=".claudio/tools/add.py"
import sys
...
```ts title=".claudio/tools/python-add.ts" {10}
import { tool } from "@claudio-code/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".claudio/tools/add.py")
