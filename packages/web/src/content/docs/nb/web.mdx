...
For å beskytte tilgang, angi et passord ved hjelp av miljøvariabelen `CLAUDIO_SERVER_PASSWORD`:

```bash
CLAUDIO_SERVER_PASSWORD=secret opencode web
```

Brukernavnet er satt til `opencode` som standard, men kan endres med `CLAUDIO_SERVER_USERNAME`.
