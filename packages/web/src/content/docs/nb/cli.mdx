...
Dette starter en HTTP-server som gir API tilgang til OpenCode-funksjonalitet uten TUI-grensesnittet. Sett `CLAUDIO_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
Dette starter en HTTP-server og åpner en nettleser for å få tilgang til OpenCode via et nettgrensesnitt. Sett `CLAUDIO_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
| ------------------------------------- | ------ | --------------------------------------------------------------------- |
| `CLAUDIO_AUTO_SHARE`                 | boolsk | Del økter automatisk                                                  |
| `CLAUDIO_GIT_BASH_PATH`              | streng | Bane til Git Bash-kjørbar på Windows                                  |
| `CLAUDIO_CONFIG`                     | streng | Bane til konfigurasjonsfil                                            |
| `CLAUDIO_TUI_CONFIG`                 | streng | Bane til TUI-konfigurasjonsfil                                        |
| `CLAUDIO_CONFIG_DIR`                 | streng | Bane til konfigurasjonskatalog                                        |
| `CLAUDIO_CONFIG_CONTENT`             | streng | Innebygd json-konfigurasjonsinnhold                                   |
| `CLAUDIO_DISABLE_AUTOUPDATE`         | boolsk | Deaktiver automatiske oppdateringskontroller                          |
| `CLAUDIO_DISABLE_PRUNE`              | boolsk | Deaktiver beskjæring av gamle data                                    |
| `CLAUDIO_DISABLE_TERMINAL_TITLE`     | boolsk | Deaktiver automatiske terminaltitteloppdateringer                     |
| `CLAUDIO_PERMISSION`                 | streng | Innebygd json-tillatelseskonfigurasjon                                |
| `CLAUDIO_DISABLE_DEFAULT_PLUGINS`    | boolsk | Deaktiver standard plugins                                            |
| `CLAUDIO_DISABLE_LSP_DOWNLOAD`       | boolsk | Deaktiver automatiske LSP servernedlastinger                          |
| `CLAUDIO_ENABLE_EXPERIMENTAL_MODELS` | boolsk | Aktiver eksperimentelle modeller                                      |
| `CLAUDIO_DISABLE_AUTOCOMPACT`        | boolsk | Deaktiver automatisk kontekstkomprimering                             |
| `CLAUDIO_DISABLE_CLAUDE_CODE`        | boolsk | Deaktiver lesing fra `.claude` (spørsmål + ferdigheter)               |
| `CLAUDIO_DISABLE_CLAUDE_CODE_PROMPT` | boolsk | Deaktiver lesing `~/.claude/CLAUDE.md`                                |
| `CLAUDIO_DISABLE_CLAUDE_CODE_SKILLS` | boolsk | Deaktiver innlasting av `.claude/skills`                              |
| `CLAUDIO_DISABLE_MODELS_FETCH`       | boolsk | Deaktiver henting av modeller fra eksterne kilder                     |
| `CLAUDIO_FAKE_VCS`                   | streng | Falsk VCS-leverandør for testformål                                   |
| `CLAUDIO_DISABLE_FILETIME_CHECK`     | boolsk | Deaktiver filtidskontroll for optimalisering                          |
| `CLAUDIO_CLIENT`                     | streng | Klientidentifikator (standard til `cli`)                              |
| `CLAUDIO_ENABLE_EXA`                 | boolsk | Aktiver Exa-nettsøkeverktøy                                           |
| `CLAUDIO_SERVER_PASSWORD`            | streng | Aktiver grunnleggende autentisering for `serve`/`web`                 |
| `CLAUDIO_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `opencode`) |
| `CLAUDIO_MODELS_URL`                 | streng | Egendefinert URL for henting av modellkonfigurasjon                   |
...
| Variabel                                        | Type   | Beskrivelse                                   |
| ----------------------------------------------- | ------ | --------------------------------------------- |
| `CLAUDIO_EXPERIMENTAL`                         | boolsk | Aktiver alle eksperimentelle funksjoner       |
| `CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY`          | boolsk | Aktiver ikonoppdagelse                        |
| `CLAUDIO_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolsk | Deaktiver kopi ved valg i TUI                 |
| `CLAUDIO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | tall   | Standard tidsavbrudd for bash-kommandoer i ms |
| `CLAUDIO_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | tall   | Maks Output Tokens for LLM-svar               |
| `CLAUDIO_EXPERIMENTAL_FILEWATCHER`             | boolsk | Aktiver filovervåker for hele dir             |
| `CLAUDIO_EXPERIMENTAL_OXFMT`                   | boolsk | Aktiver oxfmt formatter                       |
| `CLAUDIO_EXPERIMENTAL_LSP_TOOL`                | boolsk | Aktiver eksperimentelt LSP-verktøy            |
| `CLAUDIO_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolsk | Deaktiver filovervåking                       |
| `CLAUDIO_EXPERIMENTAL_EXA`                     | boolsk | Aktiver eksperimentelle Exa-funksjoner        |
| `CLAUDIO_EXPERIMENTAL_LSP_TY`                  | boolsk | Aktiver TY LSP for python-filer               |
| `CLAUDIO_EXPERIMENTAL_MARKDOWN`                | boolsk | Aktiver eksperimentelle Markdown-funksjoner   |
| `CLAUDIO_EXPERIMENTAL_PLAN_MODE`               | boolsk | Aktiver planmodus                             |
