...
OpenCode Desktop runs a local OpenCode server (the `claudio-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.
...
If you have `CLAUDIO_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `CLAUDIO_PORT` (or pick a free port) and restart.
