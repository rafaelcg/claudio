...
Uruchamia to serwer HTTP, który zapewnia dostęp do API OpenCode bez interfejsu TUI. Ustaw `CLAUDIO_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
Uruchamia to serwer HTTP i udostępnia OpenCode przez interfejs przeglądarkowy. Ustaw `CLAUDIO_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
| Zmienna                               | Typ     | Opis                                                        |
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `CLAUDIO_AUTO_SHARE`                 | boolean | Automatycznie udostępniaj sesje                             |
| `CLAUDIO_GIT_BASH_PATH`              | string  | Ścieżka do pliku wykonywalnego Git Bash w systemie Windows  |
| `CLAUDIO_CONFIG`                     | string  | Ścieżka do pliku konfiguracyjnego                           |
| `CLAUDIO_TUI_CONFIG`                 | string  | Ścieżka do pliku konfiguracyjnego TUI                       |
| `CLAUDIO_CONFIG_DIR`                 | string  | Ścieżka do katalogu konfiguracyjnego                        |
| `CLAUDIO_CONFIG_CONTENT`             | string  | Treść konfiguracji JSON (inline)                            |
| `CLAUDIO_DISABLE_AUTOUPDATE`         | boolean | Wyłącz automatyczne sprawdzanie aktualizacji                |
| `CLAUDIO_DISABLE_PRUNE`              | boolean | Wyłącz czyszczenie starych wyników (pruning)                |
| `CLAUDIO_DISABLE_TERMINAL_TITLE`     | boolean | Wyłącz automatyczne ustawianie tytułu terminala             |
| `CLAUDIO_PERMISSION`                 | string  | Konfiguracja uprawnień w JSON (inline)                      |
| `CLAUDIO_DISABLE_DEFAULT_PLUGINS`    | boolean | Wyłącz domyślne wtyczki                                     |
| `CLAUDIO_DISABLE_LSP_DOWNLOAD`       | boolean | Wyłącz automatyczne pobieranie serwerów LSP                 |
| `CLAUDIO_ENABLE_EXPERIMENTAL_MODELS` | boolean | Włącz modele eksperymentalne                                |
| `CLAUDIO_DISABLE_AUTOCOMPACT`        | boolean | Wyłącz automatyczne kompaktowanie kontekstu                 |
| `CLAUDIO_DISABLE_CLAUDE_CODE`        | boolean | Wyłącz integrację z `.claude` (prompt + skills)             |
| `CLAUDIO_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Wyłącz czytanie `~/.claude/CLAUDE.md`                       |
| `CLAUDIO_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Wyłącz ładowanie `.claude/skills`                           |
| `CLAUDIO_DISABLE_MODELS_FETCH`       | boolean | Wyłącz pobieranie modeli ze źródeł zewnętrznych             |
| `CLAUDIO_FAKE_VCS`                   | string  | Fałszywy dostawca VCS do celów testowych                    |
| `CLAUDIO_DISABLE_FILETIME_CHECK`     | boolean | Wyłącz sprawdzanie czasu modyfikacji plików (optymalizacja) |
| `CLAUDIO_CLIENT`                     | string  | Identyfikator klienta (domyślnie `cli`)                     |
| `CLAUDIO_ENABLE_EXA`                 | boolean | Włącz narzędzie wyszukiwania internetowego Exa              |
| `CLAUDIO_SERVER_PASSWORD`            | string  | Włącz uwierzytelnianie podstawowe dla `serve`/`web`         |
| `CLAUDIO_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `opencode`)     |
| `CLAUDIO_MODELS_URL`                 | string  | Niestandardowy adres URL do pobierania konfiguracji modeli  |
...
| Zmienna                                         | Typ     | Opis                                         |
| ----------------------------------------------- | ------- | -------------------------------------------- |
| `CLAUDIO_EXPERIMENTAL`                         | boolean | Włącz wszystkie funkcje eksperymentalne      |
| `CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Włącz wykrywanie ikon                        |
| `CLAUDIO_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Wyłącz kopiowanie przy zaznaczaniu w TUI     |
| `CLAUDIO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Domyślny limit czasu dla narzędzia bash w ms |
| `CLAUDIO_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Maksymalne tokeny wyjściowe dla LLM          |
| `CLAUDIO_EXPERIMENTAL_FILEWATCHER`             | boolean | Włącz obserwatora plików dla całego katalogu |
| `CLAUDIO_EXPERIMENTAL_OXFMT`                   | boolean | Włącz formater oxfmt                         |
| `CLAUDIO_EXPERIMENTAL_LSP_TOOL`                | boolean | Włącz eksperymentalne narzędzie LSP          |
| `CLAUDIO_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Wyłącz obserwatora plików                    |
| `CLAUDIO_EXPERIMENTAL_EXA`                     | boolean | Włącz funkcje eksperymentalne Exa            |
| `CLAUDIO_EXPERIMENTAL_LSP_TY`                  | boolean | Włącz TY LSP dla plików python               |
| `CLAUDIO_EXPERIMENTAL_MARKDOWN`                | boolean | Włącz funkcje eksperymentalne Markdown       |
| `CLAUDIO_EXPERIMENTAL_PLAN_MODE`               | boolean | Włącz tryb planowania                        |
