...
3. **Konfiguracja niestandardowa** (zmienna `CLAUDIO_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`opencode.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.claudio`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `CLAUDIO_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.claudio` i `~/.config/opencode` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `CLAUDIO_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `CLAUDIO_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.claudio` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.claudio`, więc **może być łączony** z ich zawartością.
...
Użyj `CLAUDIO_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/opencode/agents/` lub `.claudio/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/opencode/commands/` lub `.claudio/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.claudio/plugins/` lub `~/.config/opencode/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
