...
opencode Desktop uruchamia w tle lokalny serwer opencode (sidecar `claudio-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
Jeśli w swoim środowisku masz ustawiony `CLAUDIO_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `CLAUDIO_PORT` (lub wybierz wolny port) i uruchom ponownie.
