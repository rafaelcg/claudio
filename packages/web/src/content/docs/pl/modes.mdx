...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.claudio/plans/*.md` w celu uszczegółowienia samego planu
...
Utwórz pliki trybów w `.claudio/modes/` dla trybów zapisanych dla projektu lub `~/.config/opencode/modes/` dla trybów globalnych:
