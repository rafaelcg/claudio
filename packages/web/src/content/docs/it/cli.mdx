...
Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `CLAUDIO_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `CLAUDIO_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `CLAUDIO_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `CLAUDIO_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `CLAUDIO_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `CLAUDIO_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
| `CLAUDIO_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `CLAUDIO_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `CLAUDIO_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `CLAUDIO_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `CLAUDIO_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `CLAUDIO_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `CLAUDIO_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `CLAUDIO_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `CLAUDIO_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `CLAUDIO_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `CLAUDIO_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `CLAUDIO_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `CLAUDIO_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `CLAUDIO_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `CLAUDIO_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `CLAUDIO_DISABLE_FILETIME_CHECK`     | boolean | Disabilita controllo file time per ottimizzazione           |
| `CLAUDIO_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `CLAUDIO_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `CLAUDIO_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `CLAUDIO_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
| `CLAUDIO_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                       | Tipo    | Descrizione                                |
| ----------------------------------------------- | ------- | ------------------------------------------ |
| `CLAUDIO_EXPERIMENTAL`                         | boolean | Abilita tutte le funzionalità sperimentali |
| `CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                     |
| `CLAUDIO_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI        |
| `CLAUDIO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms  |
| `CLAUDIO_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM   |
| `CLAUDIO_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir      |
| `CLAUDIO_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                    |
| `CLAUDIO_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale         |
| `CLAUDIO_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                    |
| `CLAUDIO_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalità Exa sperimentali      |
| `CLAUDIO_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita Abilita TY LSP per i file python   |
| `CLAUDIO_EXPERIMENTAL_MARKDOWN`                | boolean | Abilita markdown sperimentale              |
| `CLAUDIO_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                          |
