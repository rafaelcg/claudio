...
:::note
Questo strumento e' disponibile solo quando `CLAUDIO_EXPERIMENTAL_LSP_TOOL=true` (oppure `CLAUDIO_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider OpenCode o quando la variabile d'ambiente `CLAUDIO_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).
