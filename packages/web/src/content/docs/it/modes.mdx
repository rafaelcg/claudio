...
- `edit` - Non puo modificare file esistenti, tranne i file in `.claudio/plans/*.md` per dettagliare il piano
...
Crea file di modalita in `.claudio/modes/` per modalita specifiche del progetto oppure in `~/.config/opencode/modes/` per modalita globali:
