...
3. **Config personalizzata** (variabile d'ambiente `CLAUDIO_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`opencode.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.claudio`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `CLAUDIO_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.claudio` e `~/.config/opencode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `CLAUDIO_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `CLAUDIO_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.claudio` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.claudio`, quindi **puo sovrascrivere** le loro impostazioni.
...
Usa `CLAUDIO_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.
...
Puoi anche definire agenti usando file markdown in `~/.config/opencode/agents/` o `.claudio/agents/`. [Scopri di piu](/docs/agents).
...
Puoi anche definire comandi usando file markdown in `~/.config/opencode/commands/` o `.claudio/commands/`. [Scopri di piu](/docs/commands).
...
Metti i file dei plugin in `.claudio/plugins/` o `~/.config/opencode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
