...
OpenCode Desktop esegue in background un server locale di OpenCode (il sidecar `claudio-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
Se hai `CLAUDIO_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `CLAUDIO_PORT` (o scegli una porta libera) e riavvia.
