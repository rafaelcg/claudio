...
const CLI_INSTALL_DIR = ".claudio/bin"
...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ? join(process.resourcesPath, `claudio-cli${suffix}`)
    : join(root, "../../resources", `claudio-cli${suffix}`)
...
export function serve(hostname: string, port: number, password: string) {
  ...
  const env = {
    CLAUDIO_SERVER_USERNAME: "opencode",
    CLAUDIO_SERVER_PASSWORD: password,
  }
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  ...
  const envs = {
    ...base,
    CLAUDIO_EXPERIMENTAL_ICON_DISCOVERY: "true",
    CLAUDIO_EXPERIMENTAL_FILEWATCHER: "true",
    CLAUDIO_CLIENT: "desktop",
