...
afterEach(async () => {
  delete process.env.CLAUDIO_CONFIG
  delete process.env.CLAUDIO_TUI_CONFIG
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".claudio"), { recursive: true })
      await Bun.write(
        path.join(dir, ".claudio", "tui.json"),
...
test("project config takes precedence over CLAUDIO_TUI_CONFIG (matches CLAUDIO_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.CLAUDIO_TUI_CONFIG = custom
    },
  })
...
test("CLAUDIO_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from CLAUDIO_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.CLAUDIO_CONFIG = path.join(customDir, "opencode.json")
    },
  })
...
test("loads .claudio/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".claudio"), { recursive: true })
      await Bun.write(path.join(dir, ".claudio", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
