...
const managedConfigDir = process.env.CLAUDIO_TEST_MANAGED_CONFIG_DIR!
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["CLAUDIO_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      opencode: {
        options: {
          apiKey: "{env:CLAUDIO_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["CLAUDIO_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["CLAUDIO_CONSOLE_TOKEN"]
    }
  }
})
...
test("loads config from .claudio directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".claudio")
...
test("loads agents from .claudio/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".claudio")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .claudio/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".claudio")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .claudio/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".claudio")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only CLAUDIO_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  process.env.CLAUDIO_CONFIG_DIR = tmp.extra
...
test("installs dependencies in writable CLAUDIO_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .claudio config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".claudio")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .claudio config with different plugins
      await Filesystem.write(
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .claudio config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets CLAUDIO_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .claudio config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .claudio directory config enables it
      const opencodeDir = path.join(dir, ".claudio")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "opencode.json"),
...
describe("getPluginName", () => {
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@claudio/plugin@2.0.0")).toBe("@claudio/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.claudio/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.claudio/plugin/oh-my-opencode.js")
  })
...
describe("CLAUDIO_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["CLAUDIO_DISABLE_PROJECT_CONFIG"]
    process.env["CLAUDIO_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["CLAUDIO_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["CLAUDIO_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .claudio/ directories when flag is set", async () => {
    const originalEnv = process.env["CLAUDIO_DISABLE_PROJECT_CONFIG"]
    process.env["CLAUDIO_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .claudio directory with a command
          const opencodeDir = path.join(dir, ".claudio", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .claudio should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["CLAUDIO_DISABLE_PROJECT_CONFIG"]
    ...
    try {
      ...
      if (originalDisable === undefined) {
        ...
        process.env["CLAUDIO_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
  ...
  test("CLAUDIO_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["CLAUDIO_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CLAUDIO_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["CLAUDIO_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["CLAUDIO_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from CLAUDIO_CONFIG_DIR, not project
...
describe("CLAUDIO_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in CLAUDIO_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CLAUDIO_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["CLAUDIO_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["CLAUDIO_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["CLAUDIO_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in CLAUDIO_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CLAUDIO_CONFIG_CONTENT"]
