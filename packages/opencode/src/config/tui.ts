...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.CLAUDIO_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.CLAUDIO_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.CLAUDIO_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".claudio") && dir !== Flag.CLAUDIO_CONFIG_DIR) continue
